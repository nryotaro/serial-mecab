...
REPOSITORY_URL := git@github.com:nryotaro/serial-mecab.git

help: ## Display this help.
	@awk 'BEGIN {FS = ":.*##"; printf "\nUsage:\n  make \033[36m<target>\033[0m\n\nTargets:\n"} /^[a-zA-Z_-]+:.*?##/ { printf "  \033[36m%-10s\033[0m %s\n", $$1, $$2 }' $(MAKEFILE_LIST)

publish: $(BUILD_DIR)setup.py $(VENV)bin/twine  ## Clone codes in the revision tagged with 'TAG' to local, build them, then publish them to the pip index server.
	cd $(BUILD_DIR) && \
	git clean -fd && \
	rm -rf serialmecab.egg-info dist build && \
	git checkout master && \
	git pull && \
	git checkout $(TAG) && \
	. $(VENV)bin/activate && \
